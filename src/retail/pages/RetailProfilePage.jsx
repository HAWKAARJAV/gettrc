  ...
  return (
    <div style={{ display: "grid", gap: 18 }}>
      <div style={{ background: "var(--c-surface)", borderRadius: RETAIL_THEME.radius.lg, border: `1px solid ${"var(--c-border)"}`, boxShadow: RETAIL_THEME.shadows.card, padding: 24 }}>
        ...
        <div style={{ fontSize: 28, fontWeight: 700, color: "var(--c-text)", marginBottom: 8 }}>{completion}% complete</div>
        <div style={{ height: 10, borderRadius: 999, background: "var(--c-surface-2)", overflow: "hidden" }}>
          <div style={{ width: `${completion}%`, height: "100%", background: `linear-gradient(135deg, ${C.gold}, ${C.goldDark})` }} />
        </div>
        <p style={{ marginTop: 12, color: "var(--c-text-muted)", fontSize: 13, lineHeight: 1.7 }}>Keep your contact details up to date. Residency and visa fields are shown for reference from your submitted eligibility file.</p>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 16 }}>
        {profileCards.map(([label, value]) => (
          <div key={label} style={{ background: "var(--c-surface)", borderRadius: RETAIL_THEME.radius.lg, border: `1px solid ${"var(--c-border)"}`, boxShadow: RETAIL_THEME.shadows.card, padding: 22 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: C.gold, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 8 }}>{label}</div>
            <div style={{ fontSize: 15, fontWeight: 700, color: "var(--c-text)", lineHeight: 1.7 }}>{value}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ background: "var(--c-surface)", borderRadius: RETAIL_THEME.radius.lg, border: `1px solid ${"var(--c-border)"}`, boxShadow: RETAIL_THEME.shadows.card, padding: 24 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 14 }}>
          {[
            ...
            <div key={key}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "var(--c-text-muted)", textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 8 }}>{label}</label>
              <input value={form[key]} onChange={(e) => setForm((prev) => ({ ...prev, [key]: e.target.value }))} placeholder={placeholder} style={{ width: "100%", padding: "14px 16px", borderRadius: RETAIL_THEME.radius.sm, border: `1.5px solid ${"var(--c-border)"}`, color: "var(--c-text)", fontSize: 14, outline: "none" }} />
            </div>
          ))}
        </div>

        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 18 }}>
          <button onClick={handleSave} disabled={saving} style={{ background: `linear-gradient(135deg, ${C.gold}, ${C.goldDark})`, color: "#fff", border: "none", borderRadius: RETAIL_THEME.radius.sm, padding: "13px 18px", fontWeight: 700, cursor: saving ? "not-allowed" : "pointer" }}>
            {saving ? "Saving…" : "Save Profile"}
          </button>
          {notice && <div style={{ alignSelf: "center", color: "var(--c-text-muted)", fontSize: 13 }}>{notice}</div>}
        </div>
      </div>
    </div>
  );
