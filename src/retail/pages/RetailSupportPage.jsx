  ...
  return (
    <div style={{ display: "grid", gap: 18, fontFamily: SANS }}>
      ...
      <div style={{ background: C.white, borderRadius: RETAIL_THEME.radius.lg, border: `1px solid ${C.border}`, boxShadow: RETAIL_THEME.shadows.card, padding: 24 }}>
        ...
        <h2 style={{ fontFamily: SERIF, fontSize: 32, color: C.navy, marginBottom: 10 }}>Support Center</h2>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 16 }}>
        <a href="https://wa.me/971000000000" target="_blank" rel="noreferrer"
          style={{ background: C.white, borderRadius: RETAIL_THEME.radius.lg, border: `1px solid ${C.border}`, boxShadow: RETAIL_THEME.shadows.card, padding: 22, textDecoration: "none", color: C.navy }}>
          ...
          <div style={{ color: C.muted, fontSize: 13, lineHeight: 1.7 }}>Fast follow-up for active applicants and payment confirmation.</div>
        </a>
        <a href="mailto:support@gettrc.com"
          ...
          <div style={{ color: C.muted, fontSize: 13, lineHeight: 1.7 }}>Send your question and include your registered email address.</div>
        </a>
      </div>
      ...
      <div style={{ background: C.white, borderRadius: RETAIL_THEME.radius.lg, border: `1px solid ${C.border}`, boxShadow: RETAIL_THEME.shadows.card, padding: 24 }}>
        ...
        <div style={{ display: "grid", gap: 12 }}>
          {[
            ...
            <div key={q} style={{ background: C.offWhite, border: `1px solid ${C.border}`, borderRadius: RETAIL_THEME.radius.sm, padding: 16 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: C.navy, marginBottom: 5 }}>{q}</div>
      ...
      <div style={{ background: C.white, borderRadius: RETAIL_THEME.radius.lg, border: `1px solid ${C.border}`, boxShadow: RETAIL_THEME.shadows.card, padding: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18, flexWrap: "wrap", gap: 12 }}>
          <div>
            ...
            <div style={{ color: C.muted, fontSize: 13 }}>Submit an inquiry and track admin replies.</div>
          </div>
          <button onClick={() => { setShowForm(true); setSelected(null); }}
            style={{ background: `linear-gradient(135deg, ${C.gold}, ${C.goldDark})`, color: C.white, border: "none", borderRadius: RETAIL_THEME.radius.sm, padding: "11px 18px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: SANS }}>
            + Raise a Ticket
          </button>
        </div>
        ...
        {/* New ticket form */}
        {showForm && (
          <div style={{ background: C.offWhite, borderRadius: RETAIL_THEME.radius.sm, border: `1px solid ${C.border}`, padding: 20, marginBottom: 20 }}>
            <div style={{ fontFamily: SERIF, fontSize: 20, fontWeight: 700, color: C.navy, marginBottom: 16 }}>New Support Ticket</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 14, marginBottom: 14 }}>
              ...
              <div>
                ...
                <select value={priority} onChange={e => setPriority(e.target.value)}
                  style={{ ...INPUT, background: C.white }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={handleSubmit} disabled={submitting}
                style={{ background: `linear-gradient(135deg, ${C.navy}, #1A3570)`, color: C.white, border: "none", borderRadius: RETAIL_THEME.radius.sm, padding: "11px 22px", fontSize: 14, fontWeight: 700, cursor: submitting ? "not-allowed" : "pointer", opacity: submitting ? .6 : 1, fontFamily: SANS }}>
                {submitting ? "Sending…" : "Submit Ticket"}
              </button>
              <button onClick={() => { setShowForm(false); setFormError(null); }}
                style={{ background: C.white, border: `1px solid ${C.border}`, color: C.navy, borderRadius: RETAIL_THEME.radius.sm, padding: "11px 18px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: SANS }}>
                Cancel
              </button>
            </div>
          </div>
        ...
        {loading ? (
          <div style={{ textAlign: "center", padding: 40, color: C.muted, fontSize: 13 }}>Loading tickets…</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: ticketById ? "320px 1fr" : "1fr", gap: 16 }}>
            {/* List */}
            <div style={{ border: `1px solid ${C.border}`, borderRadius: RETAIL_THEME.radius.sm, overflow: "hidden" }}>
              <div style={{ padding: "10px 16px", borderBottom: `1px solid ${C.border}`, fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: ".08em" }}>
                {tickets.length} ticket{tickets.length !== 1 ? "s" : ""}
              </div>
              {tickets.map(t => (
                <div key={t.id} onClick={() => setSelected(selected === t.id ? null : t.id)}
                  style={{ padding: "14px 16px", borderBottom: `1px solid ${C.border}`, cursor: "pointer", background: selected === t.id ? "#EEF2FF" : "transparent", borderLeft: selected === t.id ? "3px solid #5B21B6" : "3px solid transparent", transition: "background .15s" }}
            ...
            {/* Detail */}
            {ticketById && (
              <div style={{ border: `1px solid ${C.border}`, borderRadius: RETAIL_THEME.radius.sm, padding: 22 }}>
                ...
                <div style={{ padding: 14, background: C.offWhite, borderRadius: 10, marginBottom: 18, fontSize: 14, color: C.navy, lineHeight: 1.7, border: `1px solid ${C.border}` }}>
                  {ticketById.message}
                </div>
                ...
                {ticketById.admin_reply ? (
                  ...
                  <div style={{ marginBottom: 18, background: C.offWhite, borderRadius: 10, padding: "10px 14px", fontSize: 13, color: C.muted, border: `1px dashed ${C.border}` }}>
