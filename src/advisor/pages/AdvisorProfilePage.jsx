...
function StarRating({ rating }) {
  ...
  return (
    <div style={{ display: "flex", gap: 3, alignItems: "center" }}>
      {[1,2,3,4,5].map(i => (
        <span key={i} style={{ fontSize: 18, color: i <= stars ? C.gold : C.border }}>★</span>
      ))}
      {rating != null && <span style={{ fontSize: 13, color: C.muted, marginLeft: 6 }}>{Number(rating).toFixed(1)}</span>}
    </div>
  );
}
...
export default function AdvisorProfilePage() {
  ...
  return (
    <div style={{ display: "grid", gap: 20, fontFamily: SANS }}>
      {/* Header */}
      <div style={{ background: C.white, borderRadius: 18, border: `1px solid ${C.border}`, padding: "24px 28px", boxShadow: "0 2px 12px rgba(15,37,87,.05)", display: "flex", alignItems: "center", gap: 20 }}>
        ...
        <div>
          <h2 style={{ fontFamily: SERIF, fontSize: 26, fontWeight: 700, color: C.navy, marginBottom: 4 }}>
            {profile?.full_name || advisor?.name || "Advisor"}
          </h2>
          <div style={{ fontSize: 13, color: C.muted }}>{profile?.email || session?.user?.email}</div>
          <div style={{ marginTop: 8 }}>
            {advisor?.verified ? (
              ...
              <span style={{ background: "#F3F4F6", color: C.muted, fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 999, marginRight: 8 }}>Pending Verification</span>
            )}
            {advisor?.available && (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 18 }}>
        ...
        <div style={{ background: C.white, borderRadius: 16, border: `1px solid ${C.border}`, padding: 22, boxShadow: "0 2px 12px rgba(15,37,87,.05)" }}>
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: ".08em", display: "block", marginBottom: 6 }}>Full Name</label>
          ...
          <div style={{ marginBottom: 18 }}>
            <label style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: ".08em", display: "block", marginBottom: 6 }}>Phone</label>
            <input value={phone} onChange={e => setPhone(e.target.value)}
              style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: `1.5px solid ${C.border}`, fontFamily: SANS, fontSize: 14, outline: "none", color: C.navy, boxSizing: "border-box" }} />
          </div>

          <div style={{ marginBottom: 6 }}>
            <label style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: ".08em", display: "block", marginBottom: 6 }}>Email (read-only)</label>
            <input value={profile?.email || session?.user?.email || ""} readOnly
              style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: `1.5px solid ${C.border}`, fontFamily: SANS, fontSize: 14, color: C.muted, background: C.offWhite, boxSizing: "border-box", cursor: "not-allowed" }} />
          </div>
        ...
        <div style={{ background: C.white, borderRadius: 16, border: `1px solid ${C.border}`, padding: 22, boxShadow: "0 2px 12px rgba(15,37,87,.05)" }}>
          ...
          {advisor ? (
            <>
              ...
              <div style={{ padding: "10px 0", borderBottom: `1px solid ${C.border}` }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 4 }}>Specialties</div>
              ...
              <div style={{ padding: "10px 0", borderBottom: `1px solid ${C.border}` }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 6 }}>Rating</div>
                <StarRating rating={advisor.rating} />
                {advisor.reviews != null && (
                  <div style={{ fontSize: 12, color: C.muted, marginTop: 4 }}>{advisor.reviews} review{advisor.reviews !== 1 ? "s" : ""}</div>
                )}
              </div>
            </>
          ) : (
            <div style={{ color: C.muted, fontSize: 14 }}>No advisor profile found. Contact admin to complete your advisor setup.</div>
          )}
        </div>
        ...
        {advisor && (
          <div style={{ background: C.white, borderRadius: 16, border: `1px solid ${C.border}`, padding: 22, boxShadow: "0 2px 12px rgba(15,37,87,.05)" }}>
            ...
            {[
              ...
              <div key={tier} style={{ padding: "12px 0", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: C.navy }}>{tier}</div>
                <div style={{ fontSize: 16, fontWeight: 800, color: C.gold }}>{fee != null ? (isNaN(Number(fee)) ? fee : `AED ${Number(fee).toLocaleString()}`) : "—"}</div>
              </div>
            ))}

            <div style={{ marginTop: 16, padding: "12px 14px", background: C.offWhite, borderRadius: 10, fontSize: 12, color: C.muted, border: `1px solid ${C.border}` }}>
